...
bool FileReader::readFb2(QFile *pointerToFile){
    ...
    while( !sr.atEnd() )
    {
        ...
        case QXmlStreamReader::Characters:
            ...
            if(thisToken.contains("description")) // ОПИСАНИЕ КНИГИ
            {
                qDebug()<<"Код с описанием отрабатывает"<<thisToken.back();
                if (thisToken.contains("author"))
                {
                    author_name.append(sr.text().toString()+" ");
                }
                if (thisToken.contains("book-title"))
                {
                    book_name.append(sr.text().toString()+" ");
                }
    ...
    book.append("</ol>");
    book.append("<p>"+author_name+"</p>");
    book.append("<p>"+book_name+"</p>");
    emit opened(book);
    return ok;
}
